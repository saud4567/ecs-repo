...
// controller_name: customerProfile
// controller_description:
//      controller used to get customer profile
customerModuleControllers.customerProfile = async (req, res, next) => {
  try {
    /** Validation of request data */
    const validateBody = customerModuleValidators.customerProfile(req);

    /** handle logic within service function */
    const customerDetails = await customerModuleServices.customerProfile({
      apiKey: validateBody.apiKey,
      apiSecret: validateBody.apiSecret,
      customerRefId: validateBody.customerId,
      requestedData: validateBody.requestedData,
      requestId: req.requestId,
    });
    ...
    /**return response */
    next({
      ...customerModuleConstants.customerProfile.messages.CCDS001,
      result: customerDetails,
    });
  } catch (error) {
    if (
